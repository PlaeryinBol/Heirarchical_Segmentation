...
def mIoU(pred_mask, mask, smooth=1e-10, n_classes=len(config.CLASSES), top_level=config.TOP_LEVEL, middle_level=config.MIDDLE_LEVEL,
         bottom_level=config.BOTTOM_LEVEL):
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            if min_loss > (test_loss/len(val_loader)):
                ...
                torch.save(model, os.path.join(config.MODELS_DIR,
                                               '{}_{}_epoch_mIoU-{:.3f}.pt'.format(exp_name, e, val_iou_score/len(val_loader))))
...
def miou_score(model, device, test_set):
    ...
    for i in tqdm(range(len(test_set))):
        img, mask = test_set[i]
        pred_mask, score, top_score, mid_score, bot_score = predict_image_mask_miou(model, device, img, mask)
...
def accuracy_score(model, device, test_set):
    ...
    for i in tqdm(range(len(test_set))):
        img, mask = test_set[i]
        pred_mask, acc = predict_image_mask_pixel(model, device, img, mask)
        accuracy.append(acc)
